"""Desktop OpenCV tuner for the current MaixCAM shape detector.
...
SLIDER_GROUPS = [
    (
        "LAB 颜色阈值",
        [
            ("L_MIN", "L 最小值", 182, 0, 255, 1),
            ("L_MAX", "L 最大值", 207, 0, 255, 1),
            ("A_MIN", "A 最小值", 117, 0, 255, 1),
            ("A_MAX", "A 最大值", 169, 0, 255, 1),
            ("B_MIN", "B 最小值", 108, 0, 255, 1),
            ("B_MAX", "B 最大值", 136, 0, 255, 1),
        ],
    ),
    (
        "面积和尺寸",
        [
            ("CANDIDATE_MIN_AREA", "候选轮廓最小面积", 80, 0, 5000, 10),
            ("TRIANGLE_MIN_AREA", "三角形最小面积", 80, 0, 5000, 10),
            ("QUADRILATERAL_MIN_AREA", "四边形最小面积", 400, 0, 10000, 10),
            ("ELLIPSE_MIN_AREA", "椭圆最小面积", 180, 0, 5000, 10),
    ...
    (
        ...
        [
            ...
            ("ELLIPSE_MINOR_AXIS_MIN", "最短轴最小长度", 8.0, 1.0, 100.0, 0.5),
            ...
            ("ELLIPSE_FAR_MIN_DETAIL_SIDES", "远距离最少细节边数", 5, 3, 20, 1),
            ("ELLIPSE_FAR_MIN_TIGHT_SIDES", "远距离最少紧密边数", 4, 3, 12, 1),
            ("ELLIPSE_FAR_FILL_MIN", "远距离填充比最小值", 0.86, 0.0, 2.0, 0.01),
            ("ELLIPSE_FAR_FILL_MAX", "远距离填充比最大值", 1.18, 0.0, 2.0, 0.01),
            ("ELLIPSE_FAR_RECT_EXTENT_MAX", "远距离最大矩形占比", 0.85, 0.0, 1.0, 0.01),
            ("ELLIPSE_FAR_AXIS_RATIO_MAX", "远距离最大长短轴比", 2.20, 1.0, 10.0, 0.05),
            ("ELLIPSE_FAR_CIRCULARITY_MIN", "远距离最小圆度", 0.70, 0.0, 1.0, 0.01),
            ("ELLIPSE_FAR_SOLIDITY_MIN", "远距离最小实心度", 0.80, 0.0, 1.0, 0.01),
        ],
    ),
    (
        "识别稳定性",
        [
            ...
            ("ELLIPSE_STABLE_FRAMES_REQUIRED", "椭圆连续稳定帧数", 20, 1, 120, 1),
            ("STABLE_CENTER_MAX_STEP_PX", "中心最大跳动像素", 15, 1, 100, 1),
            ("STABLE_AREA_MAX_CHANGE_RATIO", "面积最大变化比例", 0.45, 0.0, 2.0, 0.01),
        ],
    ),
]
...
def ellipse_score(contour, params, far_ellipse=False):
    ...
    if minor_axis < params["ELLIPSE_MINOR_AXIS_MIN"]:
        return 0.0
...
class TunerApp:
    ...
    def _python_parameter_text(self):
        ...
        lines = [
            "# Generated by test2/pc_visual_tuner.py",
        ...
        for key in (
            ...
            "POLY_AREA_MIN_RATIO",
            "POLY_AREA_MAX_RATIO",
            "QUAD_EVIDENCE_AREA_RATIO_MIN",
            "QUAD_EVIDENCE_AREA_RATIO_MAX",
            "TRI_AREA_MIN_RATIO",
            "TRI_AREA_MAX_RATIO",
            ...
            "ELLIPSE_MINOR_AXIS_MIN",
            "ELLIPSE_CIRCULARITY_MIN",
            "ELLIPSE_SOLIDITY_MIN",
            "ELLIPSE_FAR_AREA_MAX",
            "ELLIPSE_FAR_MIN_DETAIL_SIDES",
            "ELLIPSE_FAR_MIN_TIGHT_SIDES",
            "ELLIPSE_FAR_FILL_MIN",
            "ELLIPSE_FAR_FILL_MAX",
            "ELLIPSE_FAR_RECT_EXTENT_MAX",
            "ELLIPSE_FAR_AXIS_RATIO_MAX",
            "ELLIPSE_FAR_CIRCULARITY_MIN",
            "ELLIPSE_FAR_SOLIDITY_MIN",
            "STABLE_FRAMES_REQUIRED",
            "ELLIPSE_STABLE_FRAMES_REQUIRED",
            "STABLE_CENTER_MAX_STEP_PX",
        ...
        if frame is not None:
            ...
            if detections:
                ...
                center_close = (
                    ...
                    and (current_center[0] - self.last_center[0]) ** 2
                    + (current_center[1] - self.last_center[1]) ** 2
                    <= params["STABLE_CENTER_MAX_STEP_PX"] ** 2
                )
                area_close = (
                    self.last_area > 0
                    and abs(current_area - self.last_area) / max(current_area, self.last_area)
                    <= params["STABLE_AREA_MAX_CHANGE_RATIO"]
                )
                if current_name == self.last_name and center_close and area_close:
